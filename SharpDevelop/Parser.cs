...
namespace scsc
{
	public class Parser
	{
		...
		public bool CheckSpecialSymbol(string symbol, bool nextToken = true)
		{
			bool result = (token is SpecialSymbolToken) && ((SpecialSymbolToken)token).value==symbol;
			if (result && nextToken) ReadNextToken();
			return result;
		}
		
		public bool CheckIdent(bool nextToken = true)
		{
		...
		public bool CheckString(bool readNext = true)
		{
			bool result = (token is StringToken);
			if (result && readNext) ReadNextToken();
			return result;
		}
...
//		}
		...
		public bool IsBlock(bool newScope)
		{
			...
			while (IsVarDecl() || IsVarAssign() || IsStatement()) ;
		...
		public bool IsVarAssign()
		{
			...
			if (!CheckIdent()) return false;
			if (!CheckSpecialSymbol("=")) Error("Очаквам специален символ '='");
			if (IsLiteral(out type, out value))
            {
				if (!AssignableTypes((symbolTable.GetSymbol(name.value) as LocalVarSymbol).localVariableInfo.LocalType, type)) Error("Различни типове на променливата");
                else if (symbolTable.GetSymbol(name.value).literal is NumberToken) (symbolTable.GetSymbol(name.value).literal as NumberToken).value = sum(type, value);
                else symbolTable.GetSymbol(name.value).literal = (value as LiteralToken);
			}
            var identToken = token;
            if (CheckIdent())
            {
                if (!AssignableTypes((symbolTable.GetSymbol((identToken as IdentToken).value) as LocalVarSymbol).localVariableInfo.LocalType, type)) Error("Различни типове на променливата");
                else if (token is SpecialSymbolToken)
                    (symbolTable.GetSymbol(name.value).literal as NumberToken).value = sum((symbolTable.GetSymbol((identToken as IdentToken).value) as LocalVarSymbol).localVariableInfo.LocalType, symbolTable.GetSymbol((identToken as IdentToken).value).literal as NumberToken);
                else symbolTable.GetSymbol(name.value).literal = symbolTable.GetSymbol((identToken as IdentToken).value).literal;
            }
            if (!CheckSpecialSymbol(";")) Error("Очаквам специален символ ';'");
		...
		public long sum(Type type, Token value)
        {
			if (typeof(System.Int32) != type) Error("Expected only numbers");
			long sum = (value as NumberToken).value;
			while (true)
			{
				if (!(token is SpecialSymbolToken)) Error("Expected special symbol");
				if ((token as SpecialSymbolToken).value == ";") break;
				
				var operation = (token as SpecialSymbolToken).value;
				ReadNextToken();

				if (!IsLiteral(out type, out value) && !CheckIdent(false)) break;
				if (CheckIdent(false))
                {
					type = (symbolTable.GetSymbol((token as IdentToken).value) as LocalVarSymbol).localVariableInfo.LocalType;
					value = symbolTable.GetSymbol((token as IdentToken).value).literal;
					ReadNextToken();
				}
				...
				{
					case "*":
						sum = sum == 0 ? (value as NumberToken).value : sum * (value as NumberToken).value;
						break;

					case "/":
						sum /= (value as NumberToken).value;
						break;
...
//			}
		...
		public bool IsStatement()
		{
			...
			if (IsLocation(out location)) {
				...
				// 
			} else if (CheckKeyword("printf")) {

				if (!CheckSpecialSymbol("(")) Error("Очаквам специален символ '('");
				if (CheckIdent(false))
				{
					Console.WriteLine(ParseLiteralToObj<string>(symbolTable.GetSymbol((token as IdentToken).value).literal));
					ReadNextToken();
				}
				else if (CheckString(false))
				{
					Console.WriteLine((token as StringToken).value);
					ReadNextToken();
				}
				else Error("Очаквам литерал");
				if (!CheckSpecialSymbol(")")) Error("Очаквам специален символ ')'");
				if (!CheckSpecialSymbol(";")) Error("Очаквам специален символ ';'");
			} else {
				return false;
		...
		public bool IsSimpleExpr(LocationInfo location, out Type type)
		{
			...
			/*} else if (IsLiteral(out type, out null)) {*/
				//
			} else {
				type = null;
				return false;
		...
		public bool IsLiteral(out Type type, out Token literal)
		{
			literal = token;
		...
		public T ParseLiteralToObj<T>(LiteralToken literal)
        {
			if (literal is null) return default;

			switch (literal)
			{
				case BooleanToken boolToken:
					return (T)Convert.ChangeType(boolToken.value, typeof(T));
				case CharToken charToken:
					return (T)Convert.ChangeType(charToken.value, typeof(T));
				case DoubleToken doubleToken:
					return (T)Convert.ChangeType(doubleToken.value, typeof(T));
				case NumberToken numberToken:
					return (T)Convert.ChangeType(numberToken.value, typeof(T));
				case StringToken stringToken:
					return (T)Convert.ChangeType(stringToken.value, typeof(T));
				default:
					return default;
			}
		}
